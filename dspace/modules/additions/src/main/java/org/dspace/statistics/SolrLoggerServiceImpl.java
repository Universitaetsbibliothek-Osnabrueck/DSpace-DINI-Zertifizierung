...
public class SolrLoggerServiceImpl implements SolrLoggerService, InitializingBean {
	...
	public static enum StatisticsType {
		VIEW("view"), SEARCH("search"), SEARCH_RESULT("search_result"), WORKFLOW("workflow");
	...
	public void afterPropertiesSet() throws Exception {
		log.debug("solr-statistics.spidersfile:" + configurationService.getProperty("solr-statistics.spidersfile"));
		log.debug("solr-statistics.server:" + configurationService.getProperty("solr-statistics.server"));
		log.debug("usage-statistics.dbfile:" + configurationService.getProperty("usage-statistics.dbfile"));
		...
		if (dbfile != null) {
			try {
				...
				log.error(
						"The GeoLite Database file is missing (" + dbfile
				...
				log.error(
						"Unable to load GeoLite Database file (" + dbfile
	...
	@Override
	public void postView(DSpaceObject dspaceObject, String ip, String userAgent, String xforwardedfor,
			EPerson currentUser) {
		if (solr == null || locationService == null) {
			return;
		}
		...
		try {
			SolrInputDocument doc1 = getCommonSolrDoc(dspaceObject, ip, userAgent, xforwardedfor, currentUser);
			if (doc1 == null)
				return;
			if (dspaceObject instanceof Bitstream) {
				Bitstream bit = (Bitstream) dspaceObject;
	...
	/**
	 * Returns a solr input document containing common information about the
	 * statistics regardless if we are logging a search or a view of a DSpace
	 * object
	 * 
	 * @param dspaceObject
	 *            the object used.
		...
		if (request != null) {
			...
			if (isUseProxies() && request.getHeader("X-Forwarded-For") != null) {
				...
				for (String xfip : request.getHeader("X-Forwarded-For").split(",")) {
					/*
					 * proxy itself will sometime populate this header with the
					 * same value in remote address. ordering in spec is vague,
					 * we'll just take the last not equal to the proxy
					 */
	...
	@Override
	public void update(String query, String action, List<String> fieldNames, List<List<Object>> fieldValuesList)
		...
		for (int i = 0; i < docsToUpdate.size(); i++) {
			...
			for (int j = 0; j < fieldNames.size(); j++) {
				...
				if (action.equals("addOne") || action.equals("replace")) {
					...
					// Remove the field
					java.util.Collection<Object> values = solrDocument.getFieldValues(fieldName);
					solrDocument.removeFields(fieldName);
	...
	@Override
	public ObjectCount[] queryFacetField(String query, String filterQuery, String facetField, int max,
			boolean showTotal, List<String> facetQueries) throws SolrServerException {
		QueryResponse queryResponse = query(query, filterQuery, facetField, 0, max, null, null, null, facetQueries,
				null, false);
	...
	@Override
	public ObjectCount[] queryFacetDate(String query, String filterQuery, int max, String dateType, String dateStart,
			String dateEnd, boolean showTotal, Context context) throws SolrServerException {
		QueryResponse queryResponse = query(query, filterQuery, null, 0, max, dateType, dateStart, dateEnd, null, null,
				false);
		if (queryResponse == null) {
			return new ObjectCount[0];
		}
	...
	public Map<String, Integer> queryFacetQuery(String query, String filterQuery, List<String> facetQueries)
		...
		QueryResponse response = query(query, filterQuery, null, 0, 1, null, null, null, facetQueries, null, false);
		return response.getFacetQuery();
	}

	@Override
	public ObjectCount queryTotal(String query, String filterQuery) throws SolrServerException {
		QueryResponse queryResponse = query(query, filterQuery, null, 0, -1, null, null, null, null, null, false);
	...
	public QueryResponse query(String query, String filterQuery, String facetField, int rows, int max, String dateType,
			String dateStart, String dateEnd, List<String> facetQueries, String sort, boolean ascending)
			throws SolrServerException {
		if (solr == null) {
			return null;
		}
		...
		if (bundles != null && bundles.length > 0) {
			...
			/**
			 * The code below creates a query that will allow only records which
			 * do not have a bundlename (items, collections, ...) or bitstreams
			 * that have a configured bundle name
			 */
			StringBuffer bundleQuery = new StringBuffer();
			// Also add the possibility that if no bundle name is there these
	...
	public void shardSolrIndex() throws IOException, SolrServerException {
		/*
		 * Start by faceting by year so we can include each year in a separate
		 * core !
		 */
		...
		// We go back to 2000 the year 2000, this is a bit overkill but this way
		// we ensure we have everything
		// The alternative would be to sort but that isn't recommended since it
		// would be a very costly query !
		...
		for (RangeFacet.Count count : yearResults) {
			...
			filterQuery.append("]");
			// The next part of the filter query excludes the content from
			// midnight of the next year !
			...
			// Tell SOLR how to escape and separate the values of multi-valued
			// fields
			...
			for (int i = 0; i < totalRecords; i += 10000) {
				...
				File csvFile = new File(tempDirectory.getPath() + File.separatorChar + "temp." + dcStart.getYearUTC()
						+ "." + i + ".csv");
			...
			for (File tempCsv : filesToUpload) {
				...
				// Add parsing directives for the multivalued fields so that
				// they are stored as separate values instead of one value
				for (String multivaluedField : multivaluedFields) {
					...
					contentStreamUpdateRequest.setParam("f." + multivaluedField + ".separator",
	...
	protected HttpSolrServer createCore(HttpSolrServer solr, String coreName) throws IOException, SolrServerException {
		...
		// return that server. Otherwise create a new one.
		...
		// Unfortunately, this class is documented as "experimental and subject
		...
		// The config files for a statistics shard reside wihtin the statistics
		// repository
		create.setInstanceDir("statistics");
		create.setDataDir(solrDir + coreName + File.separator + "data");
		// It is unclear why a separate solr server using the baseSolrUrl is
		// required.
		// Based on testing while working on DS-3457, this appears to be
		// necessary.
		HttpSolrServer solrServer = new HttpSolrServer(baseSolrUrl);

		// DS-3457: The invocation of this method will cause tomcat to hang if
		// this method is invoked before the solr webapp has fully initialized.
		// Also, any attempt to ping a repository before solr is fully
		// initialized will also cause tomcat to hang.
	 ...
	 * @throws SolrServerException
	 *             When getting the schema information from the SOLR core fails
	 * @throws IOException
	 *             When connection to the SOLR server fails
	 */
	...
	protected void addDocumentsToFile(Context context, SolrDocumentList docs, File exportOutput)
		...
		for (SolrDocument doc : docs) {
			...
			String out = time + "," + "view_"
					+ contentServiceFactory.getDSpaceObjectService(dso).getTypeText(dso).toLowerCase() + "," + id + ","
					+ outputDateFormat.format(solrDate) + ",anonymous," + ip + "\n";
